...
@pytest.mark.asyncio
async def test_trailing_stop_short_activation_fixed():
    """
    Test that SHORT trailing stop works correctly after the fix.
    """
    supabase = MagicMock()
    trader = PaperTrader(supabase)
    
    # Trade with entry=100, SL=103 -> risk=3.
    # Activation threshold = 3 * 1.5 = 4.5.
    # Price needs to reach 95.5.
    trade = SimulatedTrade(
        decision_id=uuid4(),
        symbol="BTCUSDT",
        direction="short",
        ...
        opened_at=datetime.now(timezone.utc),
        stop_loss=103.0,
        initial_stop_loss=103.0,
        atr_at_entry=2.0,
        lowest_price=100.0
    )
    
    now = datetime.now(timezone.utc)
    # Candle at 95.0 -> should activate and move SL.
    candle = Candle(
        symbol="BTCUSDT", 
        timeframe="15m", 
        open=100.0, 
    ...
    updated2 = await trader.update_trailing_stop(trade, candle2)
    assert updated2 is not None
    assert updated2.stop_loss < 99.0
