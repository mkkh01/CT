...
from .ifvg_strategy import evaluate_ifvg_signal_diagnostics
...
class BotRuntime:
    def __init__(self, settings: Settings | None = None):
        ...
        self.execution = build_execution_adapter(self.settings)
    ...
    def _persist_signal(self, signal: Signal) -> None:
        row = {key: value for key, value in signal.to_dict().items() if value is not None}
    ...
    def _persist_open_position(self, position: VirtualPosition) -> None:
        ...
        row["user_id"] = self.settings.telegram_chat_id or "local"
        row["mode"] = self.settings.execution_mode
    ...
    def _persist_closed_position(self, position: VirtualPosition) -> None:
        ...
        self._sync_queue.put(("insert_event", {"user_id": row["user_id"], "event_type": "POSITION_CLOSED", "payload": row}))
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "live_gate": self.settings.live_execution_ready(),
            "execution_health": self.execution.health(),
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
    ...
    def dashboard_snapshot(self) -> dict[str, Any]:
        ...
        try:
            ...
            overview = {
                ...
                "pid": os.getpid(),
                "execution": "disabled" if self.settings.execution_mode == "paper" else self.settings.execution_mode,
                "execution_mode": self.settings.execution_mode,
                "live_gate": self.settings.live_execution_ready(),
                "runtime_started": self._started,
                "websocket_connected": self.market.connected,
    ...
    def _on_closed_candle(self, symbol: str, interval: str, candle: dict[str, Any]) -> None:
        symbol = self._normalise_symbol(symbol)
        with self._lock:
            is_execution_candle = interval == self.settings.execution_timeframe
            # Legacy 1h callbacks are accepted only when no configured execution candles exist;
            # this preserves readiness checks without creating duplicate live decisions.
            if interval == "1h" and self.settings.execution_timeframe != "1h" and not self.market.candles(symbol, self.settings.execution_timeframe):
                is_execution_candle = True
            if is_execution_candle:
                self.cycle_count += 1
                ...
                decision_obj, diag = evaluate_ifvg_signal_diagnostics(
                    symbol,
                    candles_1h,
                    candles_4h,
                ...
                decision = {
                    "symbol": symbol,
                    "decision": decision_obj.side if decision_obj else diag.get("rejection_reason", "NO_SIGNAL"),
    ...
    def _persist_runtime_state(self) -> None:
        ...
        row = {
            "user_id": user_id,
            "execution_mode": self.settings.execution_mode,
            "live_gate_open": self.settings.live_execution_ready()[0] and self.settings.execution_mode != "paper",
            "live_gate_reason": self.settings.live_execution_ready()[1],
            "runtime_started": self._started,
            "market_data_fresh": self.market.live_data_available,
            "websocket_connected": self.market.connected,
            "user_data_stream_connected": False,
            "reconciled": self.settings.execution_mode == "paper",
            "startup_stage": self.market.status_snapshot().get("startup_stage", "unknown"),
            "selected_symbols": sorted(list(self.trader.selected_symbols)),
            "market_status": self.market.status_snapshot(),
            "metrics": {"cycle_count": self.cycle_count, "signal_count": self.signal_count, "closed_trade_count": self.closed_trade_count},
        ...
        while not self._stop.is_set():
            try:
                ...
                if op == "upsert_settings":
                    ...
                    self.supabase.insert("positions", data)
