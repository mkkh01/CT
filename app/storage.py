...
class SupabaseStore:
    ...
    def select_open_positions(self, user_id: str) -> list[dict[str, Any]]:
        result = self._request("GET", f"positions?select=*&user_id=eq.{user_id}&status=eq.OPEN&limit=20")
        return result if isinstance(result, list) else []

    def update_position(self, position_id: str, row: dict[str, Any]) -> Optional[Any]:
        return self._request("PATCH", f"positions?id=eq.{position_id}", json=row)
    ...
    def select_recent_signals(self, user_id: str, limit: int = 100) -> list[dict[str, Any]]:
        columns = "id,ifvg_id,symbol,timeframe,side,generated_at,candle_open_time,entry_price,stop_loss,take_profit,reason,risk_reward,strategy_version,metadata,status,created_at"
    ...
    def select_recent_positions(self, user_id: str, limit: int = 100) -> list[dict[str, Any]]:
        columns = "id,signal_id,symbol,side,mode,capital_allocated,entry_price,stop_loss,take_profit,quantity,opened_at,status,exit_price,closed_at,realized_pnl,fees_paid,close_reason,created_at"
        result = self._request(
            "GET",
            f"positions?select={columns}&user_id=eq.{self._user_filter(user_id)}&order=created_at.desc&limit={min(max(limit, 1), 500)}",
        )
        return result if isinstance(result, list) else []
    ...
    def insert_order(self, row: dict[str, Any]) -> Optional[Any]:
        return self.insert("orders", row)

    def insert_risk_event(self, row: dict[str, Any]) -> Optional[Any]:
        return self.insert("risk_events", row)

    def insert_candle(self, row: dict[str, Any]) -> Optional[Any]:
        return self.upsert("market_candles", row, "symbol,timeframe,open_time")
